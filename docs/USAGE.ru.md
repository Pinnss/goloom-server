...
3. Выбери **Transport**:
   ...
   - **VK TURN SRTP** (рекомендуется) — вставь ссылку на VK звонок, затем задай **Listen address (UDP)** (например `0.0.0.0:56001`, открытый в фаерволе). Сервер ретранслирует WireGuard через TURN-серверы VK под видом WebRTC-медиа — капча на сервере не нужна, ~30–40 Мбит/с. Ссылка для клиента — **`vkturnproxy://`** на карточке inbound'а (не `goloom://`). См. [VK TURN SRTP](#vk-turn-srtp) ниже.
   - **VK TURN (legacy DTLS)** — устарел; VK режет до ~7–9 КБ/с. Используй SRTP.
...
Транспорт **VK TURN SRTP** ретранслирует WireGuard через *собственные* TURN-серверы VK под видом медиа видеозвонка (RTP/SRTP). Ключевой момент: свой TURN-сервер ты не поднимаешь. Ретранслятор — это TURN VK; Goloom-сервер лишь «второй участник», на адрес которого клиент просит VK пересылать трафик. TURN VK нельзя заблокировать, не сломав звонки VK по всей стране, поэтому трафик проходит DPI, а SRTP-обёртка обходит шейп-политику VK и даёт ~30–40 Мбит/с (против ~7–9 КБ/с на легаси-DTLS).

**Создание (оператор):**

1. **+ Создать inbound** → **Transport** = `VK TURN SRTP`.
2. **VK Call link** — `https://vk.com/call/join/<id>`. Её использует только клиент (чтобы запросить TURN-креды); сам сервер в звонок не заходит.
3. **Listen address (UDP)** — публичный UDP-порт relay, например `0.0.0.0:56001`. Уникален на inbound, открыт в фаерволе.
4. Оставь авто-провижн WG включённым → **Создать**.
5. На карточке inbound'а жми **📋 vkturnproxy://** чтобы скопировать ссылку для клиента, или покажи QR.

**Подключение (клиент):** ссылку `vkturnproxy://` напрямую понимает приложение Goloom для Android (оно отправляет профиль по SRTP-relay пути: VK-auth → TURN-аллокации → DTLS-SRTP handshake), а также клиенты anton48/Moroka8 build125+. Импортируй ссылку или сканируй QR и подключайся — `goloom://` connstr для этого транспорта не нужен.

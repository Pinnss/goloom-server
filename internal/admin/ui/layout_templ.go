...
// Package ui contains the templ-based admin panel UI. Components are

// written as .templ files compiled to *_templ.go via `templ generate`

// (see Makefile target `make ui`).

//

// Layout is the top-level shell: <head> with HTMX/Alpine/Tailwind +

// dark-theme body wrapper. Concrete pages plug their content via the

// templ children mechanism.
...
//
// `x-data` on <body> bootstraps Alpine for the whole tree — without
// it, `@click` / `x-show` directives on descendants silently no-op
// because Alpine only binds nodes that have an `x-data` ancestor.
func Layout(title, bodyClass string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 1, "<!doctype html><html lang=\"ru\" class=\"bg-bg\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>")
		if templ_7745c5c3_Err != nil {
